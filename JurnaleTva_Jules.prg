...
PROCEDURE _GetPurchaseData(toEnv As Object)
	...
	TEXT TO lcSQL NOSHOW TEXTMERGE PRETEXT 15
		SELECT T.IdImport, T.IdUnic AS Id_Intrare, T.DataDoc AS Data, T.DataDocReala AS Data_Doc, T.NumarDoc AS Nr_Intrare, T.Total, T.Tva, T.Tip, T.Cont, SPACE(3) as Tip_Ded, T.IsTvaIncasare, CASE WHEN ISNULL(T.Curs, 0)=0 THEN 1 ELSE T.Curs END AS Curs
...
PROCEDURE _GetSalesData(toEnv As Object)
	...
	loEnv = toEnv
	TEXT TO lcSQL NOSHOW TEXTMERGE PRETEXT 15
		SELECT ID.IdUnic AS Id_Iesire, ID.DataDoc AS Data, ID.DataDocReala AS Data_Doc, CASE WHEN ID.DataDoc <= '2013-10-02' THEN DATEADD(day, 90, ID.DataDoc) ELSE DATEADD(day, 9999, ID.DataDoc) END AS Data90, ID.NumarDoc AS Nr, C.Denumire, C.Cod_Fiscal AS Cod_Fisc, C.Tip_Tert, C.Cont AS Cod_Tert, ISNULL(D.Valoare+D.Tva, 0) AS Total, ISNULL(D.Valoare,0) AS Baza_Tva, ISNULL(D.Tva, 0) AS Tva, CASE WHEN D.CodTva='10' THEN 'T' ELSE ISNULL(ID.Tip, '') END AS Tip, ID.IsTvaIncasare AS TvaI, CAST(1 AS NUMERIC(14,4)) AS Curs, ISNULL(D.ProcTva,0) AS Tva_Art, D.Cont, 1 AS Indice, D.CodTva AS CodTvaUser, 'Iesiri' AS Fel_D, A.CodNc, ID.IdDocStorno

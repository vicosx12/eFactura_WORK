...
FUNCTION JurnaleTva_Refactored(tnTipJurnal As Number, tdData1 As Date, tdData2 As Date, tnCategorii As Number, lOnlyInvoice As Boolean, lOnlyVanzari As Boolean)
	...
	LOCAL loEnv As Object, lcLogFile As String, _StartBegin As Number, lcProgram As String, llSuccess As Boolean, loErr As Exception, loValidation As Object
	...
	* --- Bloc principal de execuție cu gestiunea erorilor ---
	TRY
		...
		*-- Pas 2: Creare cursoare de lucru
		...
		_Log("Structura cursoarelor de lucru a fost creată.", .F., loEnv)

		*-- Pas 3: Procesare Jurnal Cumpărări (dacă este necesar)
		IF NOT loEnv.lOnlyVanzari
			_ProcessPurchases(loEnv)
			_Log("Procesarea Jurnalului de Cumpărări a fost finalizată.", .F., loEnv)
		ENDIF

		*-- Pas 4: Procesare Jurnal Vânzări
		_ProcessSales(loEnv)
		_Log("Procesarea Jurnalului de Vânzări a fost finalizată.", .F., loEnv)

		*-- Pas 5: Validare finală (non-interactivă)
		loValidation = _ValidateJournals(loEnv)
		IF loValidation.HasErrors
			_Log("Au fost identificate probleme de validare. Verificați cursorul 'crsValidationErrors'.", .F., loEnv)
		ENDIF

		_Log("Execuție finalizată cu succes.", .T., loEnv)
		llSuccess = .T.
	...
	CATCH TO loErr
		...
		*-- Logare directă, independentă de obiectul loEnv, pentru a garanta scrierea erorii.
...
FUNCTION _Init(tnTipJurnal, tdData1, tdData2, tnCategorii, lOnlyInvoice, lOnlyVanzari, tcLogFile, tnStartTime)
	...
	*-- Variabile de stare și configurare (inițializare minimă pentru logare)
	...
	*-- Șterge fișierul de log vechi și scrie antetul
	...
	ENDIF
	_Log("*** START JurnaleTva_Refactored ***", .T., loEnv)
	...
	*-- Definire constante pentru coduri TVA (îmbunătățește lizibilitatea)
	...
	#DEFINE JCODE_C_SCUTIT '10'
	#DEFINE JCODE_C_AIC_BUNURI '11-12'
	#DEFINE JCODE_C_AIC_SCUTIT '14'
	#DEFINE JCODE_C_TAX_INV_RO '17-18'
	#DEFINE JCODE_C_TAX_INV_EXTRA '15-16'
	...
	_Log("Mediul de lucru a fost inițializat.", .F., loEnv)
	RETURN loEnv
ENDFUNC
...
PROCEDURE _ProcessPurchases(toEnv As Object)
	...
	IF INLIST(toEnv.nTipJurnal, 1, 2)
		...
		_Log("Filtrul de deductibilitate (" + IIF(toEnv.nTipJurnal=1, "100%", "50%") + ") a fost aplicat.",.F.,toEnv)
	ENDIF
	IF toEnv.oSettings.ModulTvaI
...
PROCEDURE _ProcessSales(toEnv As Object)
	_GetSalesData(toEnv)
	_Log("Datele primare pentru Vânzări au fost extrase.",.F.,toEnv)
	IF toEnv.oSettings.ModulTvaI
		_GetPreviousPeriodVatOnCollectionSales(toEnv)
		_Log("Facturile cu TVA la încasare din perioadele anterioare (vânzări) au fost adăugate.",.F.,toEnv)
	ENDIF
	_ClassifySales(toEnv)
	_Log("Clasificarea înregistrărilor de vânzări a fost finalizată (set-based).",.F.,toEnv)
	IF NOT toEnv.lOnlyInvoice
		_ProcessMiscSalesFromRegister(toEnv)
		_Log("Notele contabile diverse (vânzări) au fost procesate.",.F.,toEnv)
	ENDIF
	...
	_Log("Gruparea finală a datelor de vânzări (JVanz) a fost realizată.",.F.,toEnv)
	SELECT JurnalV
	ZAP
	...
	IF toEnv.oSettings.ModulTvaI
		...
		_Log("Calculul final pentru TVA la încasare (vânzări) a fost efectuat.",.F.,toEnv)
	ENDIF
ENDPROC
...
PROCEDURE _Log(tcMessage As String, tlIsFinal As Boolean, toEnv As Object)
...
PROCEDURE _GetPurchaseData(toEnv As Object)
	...
	TEXT TO lcSQL NOSHOW TEXTMERGE PRETEXT 15
		SELECT T.IdImport, T.IdUnic AS Id_Intrare, T.DataDoc AS Data, NVL(T.DataDocReala, T.DataDoc) AS Data_Doc, T.NumarDoc AS Nr_Intrare, T.Total, T.Tva, T.Tip, T.Cont, SPACE(3) as Tip_Ded, T.IsTvaIncasare, CASE WHEN ISNULL(T.Curs, 0)=0 THEN 1 ELSE T.Curs END AS Curs
...
PROCEDURE _ClassifyPurchases(toEnv As Object)
	SELECT JC_Intermed
	REPLACE ALL Cod WITH ICASE(TvaI, ICASE(INLIST(Tva_Art, toEnv.nCotaStd, 20, 21, 24), JCODE_C_TVA_INCASARE_STD, INLIST(Tva_Art, toEnv.nCotaRed1, 11), JCODE_C_TVA_INCASARE_RED1, INLIST(Tva_Art, toEnv.nCotaRed2), JCODE_C_TVA_INCASARE_RED2, Tva_Art = 0, JCODE_C_SCUTIT, Cod ), INLIST(Tip_Tert, ' ', '1', '3') OR Tip='H', ICASE(INLIST(Tip, 'T', 'E') AND Tip_Tert='3', JCODE_C_TAX_INV_EXTRA, INLIST(Tip, 'T', 'E') AND Tip_Tert<>'3', ICASE(Tva_Art = 5, '29-30', INLIST(Tva_Art, toEnv.nCotaRed1, 11), '27-28', JCODE_C_TAX_INV_RO), INLIST(Tva_Art, toEnv.nCotaRed2), JCODE_C_RED2, INLIST(Tva_Art, toEnv.nCotaRed1, 11), JCODE_C_RED1, Tva_Art > 0, JCODE_C_STD, JCODE_C_SCUTIT ), Tip_Tert='2', ICASE(Tva_Art > 0 AND INLIST(Cont, '3', '20', '21', '22', '23', '60', '4091', '4093'), JCODE_C_AIC_BUNURI, Tva_Art > 0, JCODE_C_TAX_INV_EXTRA, Tva_Art = 0 AND INLIST(Cont, '3', '21', '22', '23', '60', '4091', '4093', '625', '471'), JCODE_C_AIC_SCUTIT, '13' ), Cod )
...
PROCEDURE _ProcessMiscPurchasesFromRegister(toEnv As Object)
	...
	loEnv = toEnv
	TEXT TO lcSQL NOSHOW TEXTMERGE PRETEXT 15
		SELECT R.Data, NVL(R.Data, R.Data) AS Data_Doc, R.Ndp, R.Suma, ALLTRIM(NVL(C.Denumire, '')) + ' ' + ALLTRIM(NVL(R.Explicatie, '')) AS Explicatie, R.ContC AS Cod_Tert, NVL(C.Cod_Fiscal, '') AS Cod_Fiscal, R.Fel_D, R.Id_Nota AS Id_Intrare
		...
		WHERE R.ContD = ?loEnv.oConturi.TvaD AND BETWEEN(R.Data, ?loEnv.dData1, ?loEnv.dData2) AND UPPER(LEFT(R.Fel_D, 2)) <> 'TR' AND UPPER(LEFT(R.Fel_D, 3)) <> 'TVA' AND Docs.IdUnic IS NULL AND (?loEnv.nCategorii = 0 OR R.IdCategorie = ?loEnv.nCategorii)
	ENDTEXT
...
PROCEDURE _CalculateVatOnCollectionPurchases(toEnv As Object)
	...
	ENDIF
	IF toEnv.oSettings.ModulTvaI
		REPLACE TvaN WITH Tva-DescFact(Id_Intrare, 2, Tva_Art, toEnv.dData2), Baza_TvaN WITH GetBazaTva(TvaN, Tva_Art), TotalN WITH Baza_TvaN+TvaN FOR TvaI AND Tva_Art <> 0
...
PROCEDURE _GetSalesData(toEnv As Object)
	...
	loEnv = toEnv
	TEXT TO lcSQL NOSHOW TEXTMERGE PRETEXT 15
		SELECT ID.IdUnic AS Id_Iesire, ID.DataDoc AS Data, NVL(ID.DataDocReala, ID.DataDoc) AS Data_Doc, CASE WHEN ID.DataDoc <= '2013-10-02' THEN DATEADD(day, 90, ID.DataDoc) ELSE DATEADD(day, 9999, ID.DataDoc) END AS Data90, ID.NumarDoc AS Nr, C.Denumire, C.Cod_Fiscal AS Cod_Fisc, C.Tip_Tert, C.Cont AS Cod_Tert, ISNULL(D.Valoare+D.Tva, 0) AS Total, ISNULL(D.Valoare,0) AS Baza_Tva, ISNULL(D.Tva, 0) AS Tva, CASE WHEN D.CodTva='10' THEN 'T' ELSE ISNULL(ID.Tip, '') END AS Tip, ID.IsTvaIncasare AS TvaI, CAST(1 AS NUMERIC(14,4)) AS Curs, ISNULL(D.ProcTva,0) AS Tva_Art, D.Cont, 1 AS Indice, D.CodTva AS CodTvaUser, 'Iesiri' AS Fel_D, A.CodNc, NVL(ID.IdDocStorno, 0) AS IdDocStorno
